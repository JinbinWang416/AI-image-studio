...
> | | 路径 | 用途 |
> |---|---|---|
> | **本目录** | `E:\...\2_开发\AI生图架构版` | **架构重构开发版**（按复盘文档重构） |
> | **原目录** | `E:\...\2_开发\图片生成` | **生产版** —— 用户日常使用，**不要动** |
>
> **两者的关系**：
> - 本目录是原目录的**代码快照**（2026-09-22）+ 即将进行的架构重构
> - **数据不共享**：本目录**没有 `output/`**（避免两份数据分叉），是干净的代码库
> - `config/settings.json` 是复制过来的，**两个目录各自独立**；改一个不影响另一个
> - 重构目标与方案见 `docs/架构重构_现状核查与实施方案.md`
>
> **在原目录（生产版）修 bug 时，记得同步到本目录**；反之亦然 —— 两边会分叉。
...
**本版额外注意**：Phase 1 之后真实实现在 `app/state/settings_store.py`，
`app/settings.py` 已不存在（生产版那边是 `app/settings.py`）。改守卫时两版要同步。
...
**本版另注**：真实现在 `app/state/`、`app/web/` 等分包里，`app/*.py` 多为转发模块；
改代码时两版都要同步，否则发布包与仓库内容会不一致。
