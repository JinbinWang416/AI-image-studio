...
from ..generation.regeneration import HOUSE_STORE_INDEX, regenerate_openai_house
from ..validation.professional import (
...
from ..state.batches import create_new_batch as create_batch, load_batch_snapshot
...
def _apply_snapshot_and_assets(cfg, snapshot: dict | None = None):
    ...
    return with_config(cfg, reference_assets=runtime_refs, effect_background_asset=runtime_effect_asset)
...
async def _start_full_run(
    ...
    *,
    ...
    if create_new_batch:
        ...
        cfg = with_config(cfg, realism_iteration=next_realism)
    ...
    async def runner() -> None:
        try:
            ...
            # 记录本次运行
            from ..generation.runs import append_run
            append_run(
                provider=cfg.provider,
                provider_label=cfg.provider_label,
                model=cfg.model,
...
def _professional_config():
    cfg = _local_validation_config()
    return with_config(cfg, output_root=LOCAL_PROFESSIONAL_ROOT, budget_limit=18)
...
async def api_generate_effect_background(payload: dict | None = None) -> JSONResponse:
    ...
    """
    from ..effect.background import generate_backgrounds
...
async def api_save_effect_params(payload: dict | None = None) -> JSONResponse:
    ...
    from ..effect.renderer import EffectParams
...
async def api_effect_params_preview(payload: dict | None = None) -> JSONResponse:
    ...
    from ..effect.renderer import EffectParams, render_storefront_glass
...
async def api_test_connection(payload: dict | None = None) -> JSONResponse:
    ...
    if name == "deepseek":
        from ..state.settings_store import is_masked
    ...
    if name == "flux_local":
        ...
        if requested_url:
            try:
                ...
                raise HTTPException(status_code=400, detail=str(exc)) from exc
            cfg = with_config(cfg, base_url=safe_url)
...
async def api_prompt_versions() -> JSONResponse:
    ...
    from ..prompt.templates import current_version, preview, version_summary
...
async def api_regenerate(payload: dict | None = None) -> JSONResponse:
    ...
    force_cfg = with_config(cfg, overwrite=True)
...
async def api_runs() -> JSONResponse:
    """运行历史与累计统计。"""
    from ..generation.runs import load_runs, summary
